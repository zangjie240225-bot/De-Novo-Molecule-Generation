#不停生成符合条件的新分子，每轮生成1000个候选分子，直到使用CTRL+C 终止程序运行。使用方法：python denovo_loop.py

import os
import sys
import pandas as pd
from time import time
from datetime import datetime
from tdc import Oracle, Evaluator
from genmol.sampler import Sampler
from rdkit import Chem
from rdkit.Chem import Descriptors, Lipinski

BATCH_SIZE = 1000  # 每次生成多少个候选分子

def check_props(smiles):
    """检查分子是否符合 CNS-like 条件"""
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return None

    mw = Descriptors.MolWt(mol)
    logp = Descriptors.MolLogP(mol)
    tpsa = Descriptors.TPSA(mol)
    hbd = Lipinski.NumHDonors(mol)

    if not (240 <= mw <= 300):
        return None
    if not (2 <= logp <= 3):
        return None
    if not (tpsa <= 70):
        return None
    if not (1 <= hbd <= 2):
        return None

    return {"MolWt": mw, "LogP": logp, "TPSA": tpsa, "HBD": hbd}

if __name__ == "__main__":
    evaluator = Evaluator("diversity")
    oracle_qed = Oracle("qed")
    oracle_sa = Oracle("sa")
    sampler = Sampler("model.ckpt")

    results = []
    total_generated = 0
    t_start = time()

    try:
        while True:  # 无限循环
            samples = sampler.de_novo_generation(
                BATCH_SIZE, softmax_temp=0.5, randomness=0.5
            )
            print(f"[DEBUG] 本轮生成 {len(samples)} 个分子")
            total_generated += len(samples)

            if not samples:  # 如果没生成任何分子
                print("警告: 本轮 sampler 没有生成分子，请检查模型或参数。")
                continue

            df = pd.DataFrame({
                "smiles": samples,
                "qed": oracle_qed(samples),
                "sa": oracle_sa(samples),
            })
            df = df.drop_duplicates("smiles")

            # 先 QED/SA 过滤
            df = df[(df["qed"] >= 0.6) & (df["sa"] <= 4)]

            # 再理化性质过滤
            for smi, qed, sa in zip(df["smiles"], df["qed"], df["sa"]):
                props = check_props(smi)
                if props:
                    results.append({
                        "smiles": smi,
                        "qed": round(qed, 2),
                        "sa": round(sa, 2),
                        **{k: round(v, 2) for k, v in props.items()},
                    })

            print(f"当前已收集 {len(results)} 个合格分子，累计生成 {total_generated} 分子")

    except KeyboardInterrupt:
        print("\n检测到手动停止，正在保存结果...")

    # 保存结果，加时间戳避免覆盖
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"denovo_results_{timestamp}.csv"
    df_final = pd.DataFrame(results)
    df_final.to_csv(output_file, index=False)

    print(f"\n共收集 {len(results)} 个合格分子，结果已保存到 {output_file}")
    print(f"总耗时 {time() - t_start:.2f} 秒，累计生成 {total_generated} 分子")
